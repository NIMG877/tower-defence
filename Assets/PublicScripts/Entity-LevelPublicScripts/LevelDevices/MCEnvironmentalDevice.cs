...
/// <summary>MC 主题饥饿系统设备（普通类，非 MonoBehaviour）：由 LevelResourceSharing
/// 按 LevelData.EnvironmentalControlDevice 枚举在 LevelInitialize 实例化并驱动 IManagerStartEnd。</summary>
public class MCEnvironmentalDevice : IManagerStartEnd
{
    /// <summary>
    /// 饥饿系统(字段是 HungryValue,随时间每秒衰减 0.15,钳在 0~130):
    /// HungryValue>100 时未满血每秒扣 200 血并回落 2 点;80<HungryValue<=100 时未满血每秒扣 40 血并回落 0.4 点;
    /// HungryValue==0 时每秒扣 50 血。增减益档位:>100 攻击+15%/攻速+30;30<HungryValue<=100 无增减;<=30 攻击-15%/攻速-30。
    /// </summary>
    private class EntityAndHungryMessage
    {
        ...
        /// <summary>
        /// 状态种别(构造时传入,当前 ToStart 固定 0):作 StatImages 图标组下标,
        /// 为 1 时攻击成功每次多扣 5 点饥饿值(否则 1)。饥饿增减益档位不取
        /// 决于它,由 Update() 按 HungryValue 阈值切换。
        /// </summary>
    ...
    public void ToStart()
    {
        ...
        EntityManager.Manager.OnAfterSetEntity += (Entity setEntity) =>
        {
            if (setEntity.Camp == 1 && setEntity.TryGetComponent(out InteractableStatic staticEntity) && setEntity.EntityData.CharacterJob != 8)
            {
                ...
                setEntity.Attack.OnAttackSuccessfully += () =>
                {
