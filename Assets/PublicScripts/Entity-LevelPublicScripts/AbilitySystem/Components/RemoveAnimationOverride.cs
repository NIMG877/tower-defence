...
{
    [RegisterComponent("RemoveAnimationOverride")]
    public class RemoveAnimationOverride : AbilityComponentBase
    {
        ...
        public override void OnTrigger(AbilityContext ctx)
        {
            ...
            List<AnimationOverrideRecord> records =
                ctx.sharedBlackboard.Get<List<AnimationOverrideRecord>>(inputKey, null);
            if (records == null)
            {
                Debug.LogWarning($"RemoveAnimationOverride: no override records at blackboard key '{inputKey}'");
                return;
            }
            ...
            {
                AnimationOverrideRecord record = records[i];
                if (record == null || !targetSet.Contains(record.Target)) continue;
                record.Target?.entityAM?.RemoveOverride(record.Handle);
                records.RemoveAt(i);
            }

            if (records.Count == 0)
            {
                ctx.sharedBlackboard.Remove(inputKey);
            }
            else
            {
                ctx.sharedBlackboard.Set(inputKey, records);
            }
        }
