"""技能语料访问:数据源 = 组件库 ability.db 的 skills 表(Unity 菜单导出的
skills.json 经 db/import_skills.py 导入,Unity 资产仍是真相源)。
...
ability.db 按 mtime 惰性重载（ability_db.get）：重建库后无需重启服务。缺失/
损坏 → 返回 None，调用方全部静默降级为无语料（不检索、不统计、不查撞名）。
"""
...
def get_corpus(db_path: str | None) -> dict | None:
    """语料访问入口（mtime 缓存在 ability_db）。返回 {"skills": [...], "existingIds": set}。"""
    snap = ability_db.get(db_path)
    if snap is None:
        return None
    return {"skills": snap.skills, "existingIds": snap.existing_ids}


def skill_ops(skill: dict) -> set[str]:
    """技能用到的组件 op 集合（递归 steps/elseSteps，排除原语；op 原样收集）。"""
    ops: set[str] = set()
...
def search_skills(corpus: dict, query: str, limit: int = 5) -> list[dict]:
